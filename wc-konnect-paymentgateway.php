...
function init_wc_konnect_gateway()
{
    ...
    class wc_konnect_gateway extends WC_Payment_Gateway
    {
        public function __construct() {
            ...
            $this->wire_transfer = $this->settings['wire-transfer'];
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                    ...
                    'wire-transfer' => array(
                        'title' => __('wire-transfer', 'konnect-payments'),
                        'type' => 'checkbox',
                        'label' => __('Activer paiement par virement bancaire', 'konnect-payments'),
                        'default' => 'no'
                    ),
                    'auto-complete-order' => array(
                        'title' => __('auto-complete-order', 'konnect-payments'),
                        'type' => 'checkbox',
                        'label' => __('Auto set order to complete when payment is successful', 'konnect-payments'),
                        'default' => 'no'
                    ),
                );
        }
        ...
        public function generate_konnect_form($order_id) {
            ...
            // echo '<pre>'.print_r( $currency_code, true ).'</pre>';
            // wc_add_notice(sprintf(__($currency_code, 'konnect-payments')), $notice_type = 'error');
            ...
            //     $products[] = (object) ['name' => $item["name"],'quantity'=>$item["quantity"]];
            // endforeach;
        ...
        public function successful_request() {
            ...
            wc_add_notice(sprintf(__('Erreur de paiement.', 'konnect-payments')), $notice_type = 'error');
            wp_redirect(get_permalink(get_option('woocommerce_checkout_page_id')));
            exit;
        }
